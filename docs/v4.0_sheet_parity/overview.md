...
- [ ] *(plan §3)* **Stat identity pass** — APT becomes **ATP** ("Actions per turn"), stats gain the
  sheet's flavour descriptions and Physical/Mental/Vitals grouping.
  → [systems/03-stats-and-vitals.md](./systems/03-stats-and-vitals.md)
- [ ] *(plan §4)* **Races ×25 with creature identity** — the picker grows from 10 races to 25, and
  a race now carries creature type, size and challenge rate.
  → [systems/04-races.md](./systems/04-races.md)
- [ ] *(plan §5)* **Skills re-scaled, focus skills return** — every skill gets a primary (0.35 or
  ...
  +woodcrafting). → [systems/06-skills-and-focus.md](./systems/06-skills-and-focus.md)
- [ ] *(plan §6)* **Equipment slots become the six-slot body** — Head/Upperbody/Lowerbody/Foot
  gear plus right and Left hand; the accessory box is gone.
  → [systems/08-equipment-slots.md](./systems/08-equipment-slots.md)
...
- [ ] *(plan §10)* **Item composition** — a carried thing is *material tier + template + inlay
  tier* ("Iron Ore 10 Battleaxe with Diamond 4 inlay"); its stat side is the material's vector plus
  the inlay's, its skill side is the template's — arithmetic confirmed against the sample
  character. → [systems/12-item-composition-and-backpack.md](./systems/12-item-composition-and-backpack.md)
- [ ] *(plan §11)* **Spells: entity and compendium** — 418 spells with mana cost, range/time and
  effect text; learned-per-character tracking; casting spends the existing Mana pool.
...
- [ ] *(plan §14)* **Progression additions** — Dream level as new hand-set player state **that
  amplifies archetype gains** (main × dream, sub + dream); one shared point pool
  (level × 3) covering stat *and* skill investment; the Points to Use / Points Spend readout; the
  Temp boxes on vitals. → [systems/02-progression-and-identity.md](./systems/02-progression-and-identity.md)
...
Ordering reasons: §1–§4 are small data/constant revisions that unblock fragment work and carry no
reshapes. §5 reworks the skill engine before anything that grants skill bonuses exists (§9–§10
target skills). §6 must land before §10 because composed items hang off the six slots. §7–§8 are
the ingredients of §10's composition. §11–§12 need Mana (already a resource) and the §5 skill list.
§13–§14 are additive and independent. §15 closes the milestone the way DX-04 closed v2.0.

...
| An unaffordable cast — refuse, or let the pool go negative? ([13](./systems/13-spells.md)) | **Refuse, naming the shortfall** (2026-08-31, TICKET-SPL-02). Deliberately unlike `set-resource`, which stays open at the bottom (Req 14.4). |
| Which pool does a cast spend from? ([13](./systems/13-spells.md)) | **The Player names it at cast time** (2026-08-31, TICKET-SPL-02) — no ruleset field, no stat matched by the spelling *Mana*. |

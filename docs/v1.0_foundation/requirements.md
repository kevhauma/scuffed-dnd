...
4. ~~THE Application SHALL allow Players to select a Focus_Stat~~ — **RETIRED (TICKET-ARC-03)**, and replaced by the wizard's archetype step; see Requirement 9. **Still retired after TICKET-SKL-05**, which added a *focus skill* step to the same wizard: that is the new workbook's own mechanic — three slots, each multiplying a skill's growth, duplicates stacking (v4 systems/06) — and not this criterion's one flat bonus on one Focus_Stat. The word is shared; nothing else is

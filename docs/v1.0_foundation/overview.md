...
- [ ] [TICKET-ROLL-01](./tickets/TICKET-ROLL-01-dice-rolling-engine.md) — Dice simulator and combat roll aggregator (Req 5.5, 5.6) — pure engine, no UI; the last unbuilt piece of the logic layer, and TICKET-ROLL-02 plus the sheet's roll buttons both depend on it
...
- [ ] Config dashboard with validation status and a "Validate Configuration" action *(plan §17.2 — Req 18.5, 18.6, 21.1-21.5)* — independent, can ship any time; `engine/validator.ts` and the `ValidationReport` primitive both already exist

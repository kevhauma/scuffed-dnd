...
- [x] [TICKET-CHAR-01](./tickets/TICKET-CHAR-01-character-list.md) — Character list at `/play` (Req 11.1, 17.4, 21.1-21.5) — the entry point to play mode; nothing else in play mode is reachable until it exists
- [x] [TICKET-CHAR-02](./tickets/TICKET-CHAR-02-character-creation-wizard.md) — Character creation wizard at `/play/create` (Req 11.1-11.6, 21.1-21.5) — needed TICKET-CHAR-01 for its entry point, TICKET-CALC-01 for its review step, and TICKET-SKL-01 for point allocation; produces the characters every later play-mode ticket renders
- [x] [TICKET-NAV-01](./tickets/TICKET-NAV-01-root-layout-and-mode-switching.md) — Root layout: medieval shell, real mode switching, play-mode config lock (bug fix, Req 19.1-19.6, 22.1-22.6 — `__root.tsx` is still stock scaffold and `useUIStore.mode` is never written) — after the first play screens exist so the play nav has destinations, but before the §17.5 polish pass
- [x] [TICKET-UI-01](./tickets/TICKET-UI-01-base-component-convention-cleanup.md) — Base component library cleanup: drop parent-layout constraints, replace `bg-white`/hex literals with theme tokens, add `FormField.style.ts`, fix the barrels (refactor, Req 21.2/21.3/21.6/21.7, 22.1, 22.4) — independent, can ship any time; cheaper before play mode adds a second wave of call sites, and closes most of §17.6 early
- [x] [TICKET-CHAR-03](./tickets/TICKET-CHAR-03-character-sheet.md) — Character sheet at `/play/character/$id` with the embedded StatEditor — header, racial modifiers, main skills, current/max stats, speciality and combat skills (Req 8.5, 13.4, 14.1-14.5, 9.3, 21.1-21.5) *(covers plan §12.3 and §12.6)* — needs TICKET-CHAR-02, which navigates here; combat **roll buttons** stay with the CombatSkillRoller line below. Landed 2026-08-01 with its browser criterion **still open** at the User's request — the automated suite covers the rest
- [x] [TICKET-INV-01](./tickets/TICKET-INV-01-inventory-panel-and-equipment-wiring.md) — Inventory panel on the character sheet, with slot-type validation enforced in the store (feature + bug fix, Req 12.1-12.6, 13.1-13.3, 13.5 — `useCharacterStore.equipItem()` writes any item into any slot with no type check today) *(covers plan §12.4 and §14.1)* — needs TICKET-CHAR-03 to hang off; §14.1 needed no new calculation, only the equip action and a test that the sheet's numbers move. Landed 2026-08-01 with its browser criterion **still open** at the User's request; drag-and-drop was deliberately not built (see the ticket)
- [x] [TICKET-ROLL-02](./tickets/TICKET-ROLL-02-combat-skill-roller.md) — Combat skill roller on the character sheet: per-skill roll button, result breakdown, session history (Req 15.1-15.5, 5.5, 5.6, 21.1-21.5, 22.1-22.6) *(covers plan §12.5)* — needs TICKET-ROLL-01 for the engine and TICKET-CHAR-03 for the section it mounts into; `rollCombatSkill` and the whole `useUIStore` roll-history surface existed with no callers. Landed 2026-08-01 with its browser criterion **still open** at the User's request — the settle animation is the part only a browser confirms
- [x] Play mode routes wired to real components *(plan §13.3 — Req 19.5)* — closed 2026-08-01 by TICKET-CHAR-01/02/03: all three play routes render a real component (`CharacterList`, `CharacterCreationWizard`, `CharacterSheet`) and `src/routes/play/playRoutes.test.tsx` asserts each one mounts it, carries no scaffold copy, and uses no stock palette classes
- [x] [TICKET-VAL-01](./tickets/TICKET-VAL-01-config-dashboard-validation-status.md) — Validation status and a "Validate Configuration" action on the config dashboard (Req 18.5, 18.6, 18.1-18.4, 21.1-21.5) *(covers plan §17.2)* — independent, can ship any time; `engine/validator.ts`, the `ValidationReport` primitive and `useUIStore.setValidationReport` all already exist with no callers. **Still needed after TICKET-FORM-01**, which guards the *dialogs*: an imported configuration never passes through them, and per FORM-01's implementation note a multi-formula cycle can only enter that way. This is the check that catches it. Landed 2026-08-01 with its browser criterion **still open** at the User's request
- [x] [TICKET-IO-02](./tickets/TICKET-IO-02-export-import-and-rename-configuration.md) — Export, import and rename the configuration from config mode (Req 1.4, 1.5, 1.6, 1.1, 18.5, 21.1-21.5) *(covers plan §15.1 and §15.2)* — independent; the whole `services/importExport.ts` surface exists with no callers, and this is where the deferred **rename field** from the "one configuration at a time" decision lands. Landed 2026-08-01 with its browser criterion **still open** at the User's request — and here that gap has weight, since jsdom cannot exercise the actual file download
- [x] [TICKET-CUR-01](./tickets/TICKET-CUR-01-currency-conversion-engine.md) — Currency conversion in the engine, and values shown in the right tier (refactor + feature, Req 10.4, 10.5, 10.2, 10.3, 21.1-21.5) *(covers plan §16.1)* — independent; conversion currently lives as a loop inside `ConversionCalculator`, the one piece of arithmetic that never went through the engine, and Req 10.4 was unmet — `MaterialCard` showed the raw stored amount in the raw stored tier. Landed 2026-08-01 with its browser criterion **still open** at the User's request

...
- [ ] [TICKET-LIVE-04](./tickets/TICKET-LIVE-04-membership-events-and-the-member-list-applier.md) — Membership Events and the member-list applier (v3 Req 44.3, 44.4, 44.7; 39.3–39.5) — **needs TICKET-DM-04**, and is the scope two tickets each measured and correctly declined: GAM-04's four membership writes reach no Event, so another Member's roster is stale about who is at the table until they reload. LIVE-02 deferred it on a *duplication* argument that DM-04 spent; DM-04 deferred it on a better one — `applyEventToCharacter` answers `stale` for a type it cannot apply, so a new Event type without an applier decision **refetches every open sheet on every join and leave**. That hazard is this ticket's first criterion. No migration: `SESSION_EVENT.SNAPSHOT_REFRESHED` is the precedent for a table-level Event

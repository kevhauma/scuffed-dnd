...
| AUTH-03 | `auth/guards.ts` — the five guards every later route spends one line on (GAM-03 added `requireInvitee`, PLY-01 `requireCharacterPlayer` and DM-01 `requireCharacterDM` — the writer rule minus the DM and minus the owner respectively), 401-before-lookup and one 404 after; `repositories/gameSessionRepository` + `characterRepository`; `routes/routeGuards.test.ts`, which fails on a handler that names an owned resource and calls no guard ✅ |
...
| LIVE-01 | `ws/` — `rooms.ts` (one room per Game_Session, in memory, importing `ws` **not at all**, which `the-socket-library-has-one-importer` enforces), `subscription.ts` (the socket's whole inbound surface: `subscribe`/`unsubscribe`, guarded by `requireMember`, everything else dropped and logged), `liveSocketServer.ts` (`attachLiveSocket`, the only module that imports `ws`). Authentication is the same `accountFromRequest` the pipeline uses, run **on the upgrade**, and `routeGuards.test.ts` grew a second corpus keyed on `CLIENT_MESSAGE_TYPE.` so the socket is held to the guard rule too. Two things a reader should not have to rediscover: a refused connection still gets an **`'error'` listener** (without one, a malformed frame from an anonymous client ends the process), and what stops a cross-site handshake is the cookie's **`sameSite: lax`**, recorded in that module's header. Attached by two callers, neither of them `entry.ts` — a Vite plugin under `yarn dev`, `serve.ts` against the build (POL-03) ✅ |
| LIVE-02 | `events/recordEvent.ts` — **the only code path that inserts into `event`**, and it broadcasts every row it writes through `rooms.broadcast`, so an action cannot change a character without the table being told. It **injects** the appender into `recordPlayerAction` and `refreshSessionSnapshot` rather than letting them import one, which is what makes `appendEvent(`/`appendEventWithin(` a single call site that `events/eventFanOut.test.ts` asserts as an equality rather than an allow-list. It cannot open the transaction — `queries-belong-to-repositories` keeps it away from Drizzle — so the repository writes and returns both rows and the publish happens after commit, wrapped, because a fan-out that threw would report a committed change as a failed one ✅ |
| LIVE-03 | presence, reconnect and replay |
| POL-03 | `serve.ts` — the **production listener**, and the last thing the milestone was missing: one `node:http` server that serves `dist/client/`, bridges everything else to `entry.fetch`, and calls `attachLiveSocket` on itself, so a built artefact has the socket LIVE-01 could only give `yarn dev`. `http/nodeBridge.ts` translates `IncomingMessage`/`ServerResponse` ⇄ `Request`/`Response` by hand (the load-bearing line is `getSetCookie()`, since iterating `Headers` combines `Set-Cookie` into one value no browser splits back) and is now also how the socket upgrade becomes a `Request`; `http/staticFiles.ts` serves the bundle, refusing traversal on the **resolved** path. `db/backup.ts` is `VACUUM INTO`. `entry.ts` gains `start()` and re-exports `backupDatabase`, because the build emits one server file and that file is the only door `scripts/serve.mjs` and `scripts/backup.mjs` have ✅ |

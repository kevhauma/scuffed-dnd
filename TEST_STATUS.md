...
**The net is +62 and the gross is +86, because 23 cases moved rather than being written.**
`SessionLobby.test.tsx` (15) and `SessionCharacters.test.tsx` (10) were **deleted with their
components**, and every case in them reappears in `roster/SessionRoster.test.tsx` — **two of them
folded into a case that was already there** rather than surviving as cases of their own: the
character panel's *shows a refusal where the reader is looking*, which the lobby asserted
identically, and its *says nothing is here rather than showing an empty list*, whose behaviour is
now one assertion inside *names everybody with their role*. So 25 cases became 23. That is the
honest reading of a ticket whose eighth criterion is *there is exactly one member list*: the suite
should show a merge, not an addition.

**One case did not move, it widened, and that is worth naming rather than filing as a move.** The
character panel's *offers a sheet on your own character and on nobody else's* is now *…and on
anybody's if you run the table* — a deliberate capability change that DM-04 makes, not a rewording.
A DM reading a player's sheet is DM-05's read-only view, which did not exist when the old case was
written.

**Where the new tests went.** `roster/rosterView.test.ts` +14 (the pure derivation: a Snapshot that
gains a resource gaining a column *and* two quick actions, the level chipping when the curve cannot
price it, the departed group derived from who is seated and keeping its numbers, the stable order),
`roster/useRosterFeed.test.ts` +11 (the three paths criterion 4 names — a DM's adjustment patching
with no request, four point spends costing **one** re-read, and a roll costing neither — plus the
Snapshot refresh that re-reads the *rules*, and two Events in one tick composing rather than the
second losing the first's patch), `roster/rosterQuickActions.test.tsx` +7 (v3 Req 49.7 and 49.10),
`roster/useSessionRollLog.test.ts` +8, `roster/oneMemberList.test.ts` +4,
`play/shared/characterQuickActions.test.ts` +10, `SessionList.test.tsx` +1,
`play/rolls/useRoller.table.test.tsx` +3, `play/rolls/RollHistoryPanel.test.tsx` +2, and
`play/dm/noResourceVocabulary.test.ts` unmoved at 4 but scanning **nine** modules where it scanned
five.

**Two of the new files are enforcement rather than coverage, and both were verified by mutation.**
`oneMemberList.test.ts` walks the whole client tree and fails on any module outside `roster/` naming
`SessionMemberListing`, `SessionMemberSummary`, **or calling `useSessionMembers`**; adding that
import to `SessionsPanel.tsx` turns it red, and removing it turns it green.

**The third pattern is the review's, and it is the one that made the check mean its name.** The
first two catch only a surface that names a *type*, and the shortest path to a second lobby names
neither: destructure the hook, `.map` the rows, let TypeScript infer the element type. The mutation
originally used to verify the check — adding a type import — was the one the patterns were written
for, which is the third time in this milestone a source-scanning test has been verified against its
own shape (LIVE-02 and DM-03 were the others). Two later spellings were wrong in opposite
directions and are worth recording, because both looked right: a bare `\buseSessionMembers\b` went
red on the two docblocks that *discuss* the hook, and a scan that fails on prose teaches people to
stop writing prose; matching the import path instead leaked through `sessions/index.ts`, which
re-exports it, so a consumer importing from the barrel stayed invisible. **The call is the thing no
consumer can avoid and no comment can perform.** Its first draft also grepped for the route path `/members` and
went red on `characterStore.ts` — which names the route in a **comment explaining why it does not ask
it**, the exact opposite of a second member list. The lesson is `referenceArms.test.ts`' own, met
again: *a test that greps source is coupled to the source's punctuation*, so grep for the fewest
distinctive tokens that can carry the meaning. The types are the honest signal; a listing a module
cannot type is a listing it cannot render.

**`noResourceVocabulary.test.ts` went red on the refactor, which is what its loud-failure anchor was
for.** DM-03 scanned a *slice* of `useCharacterSheet.ts` between two string anchors, because the file
could not be scanned whole — its own header reads *"Character Sheet **Mana**ger Hook"*, a false
positive on line 2. DM-04 moved that derivation out to `characterQuickActions.ts` so both placements
could share it, the anchors stopped matching, and the check **failed by name** rather than silently
scanning a slice that had ceased to exist. The module *is* the derivation now and is scanned end to
end, which retires the fragility along with the anchors.

**`fallow` charged one complexity finding and it was split.** `useRosterFeed` came back at **3
cyclomatic / 17 cognitive** against a threshold of 15 — *a reading on an intermediate state, not
reproducible from the shipped tree* — and `useCoalescedReads` is the answer — *what
an Event means to a character* and *when to ask the server again* are two subjects, and the second one
is the timing machinery (`useTableCharacterFeed`'s shape) that has nothing to do with the applier. It
is the DM-05 and LIVE-03 split, made on the same grounds and measured the same way; after it the
changed-code audit reports **one** high-complexity function, and it is the inherited one.

**Two findings in the audit are inherited and are not this ticket's**, both checked rather than
assumed. The unused type `RulesetHomeKind` is on `main` at `rulesetSync.ts:59` (`git show main:` says
so) and `fallow` reports the unused `fallow` dependency with `introduced: false`. The one CSS finding
— `grid-cols-[minmax(0,1.7fr)_minmax(0,1fr)]` "introduced since main" at `CharacterSheet.tsx:201` — is
the same line at `main`'s 200, attributed only because the file is in the changed set; this ticket's
whole diff there is a destructured field, a prop and a comment.

## TICKET-LIVE-03 — +91 tests, +5 files (4092 → 4183), and two complexity findings split rather than suppressed

**Where the +91 went.** The ticket has two halves and so does its suite. *Reconnect and replay*:
`services/liveSocket.test.ts` +25 (the reconnect, driven by `vi.useFakeTimers` and an **injected
random** — the backoff, the two refusals to retry, the reconciliation on open, duplicate suppression,
and fifty clients coming back at fifty different moments), `services/liveBackoff.test.ts` +7 (a new
file, the pure delay), `ws/replay.test.ts` +9 (a new file: exactly the missed Events, the window at
its edge, and the resynchronise instruction), `ws/subscription.test.ts` +10 and
`repositories/eventRepository.test.ts` +4. *Presence and the surface*: `ws/rooms.test.ts` +7,
`components/live/presenceState.test.ts` +6, `components/live/useLiveRoom.test.ts` +7 and
`components/live/LiveStatusNotice.test.tsx` +6 (three new files), `SessionLobby.test.tsx` +2,
`useTableCharacterFeed.test.ts` +4, `characterStore.table.test.ts` +2, `eventFanOut.test.ts` +2.

**Eight of those tests are the review's, and one of them covers a gap in the criterion the ticket
exists for.** The client resumed a room from *the last `seq` it had seen*, so a Player at a **quiet**
table had no resume point at all: the reconnect asked for nothing, the server answered *you are in
that room* and nothing else, and an adjustment made while they were away was neither replayed nor
refetched — the sheet stayed wrong with no correction pending, which is the exact failure the ticket
describes in its own Notes. The synchronous-turn property was never in doubt and never covered this:
it guarantees no gap *between* the join and the query, and says nothing about a subscribe that asks
for no query. The fix removes the special case rather than refining it — `SubscribedMessage` carries
where the log stands, a room adopts it once, and `resumeFrom: null` now means *first subscribe*
rather than *seen nothing*. **Mutation-verified**: restoring the `> 0` test fails *should resume a
quiet table from zero rather than asking for nothing* and nothing else.

**Three of them are about a *number of times* rather than a value, which is the shape LIVE-02's
review made this file's rule.** *Fifty clients do not come back together* is asserted as fifty
distinct delays with a counter-based fake random — against `Math.random` it would be a distribution,
which is to say a flake. *Says nothing at all about a second tab of the same Account* needs one
person holding **two** connections, because a registry counting sockets instead of people passes
every other case in `rooms.test.ts`. And *reads again for an Event that applied while it was reading*
was **verified by mutation**: disabling the one line it covers turns it red, and it is the only case
in that file that moves.

**The one-synchronous-turn property is asserted in the only form it has.** `subscription.test.ts`'s
*should admit and catch up in one synchronous turn* awaits nothing and asserts that the room holds
the connection **and** the replay has been delivered by the time `handleClientMessage` returns. That
is what makes the replay gapless — no Event can be written between the join and the query, because
there is no instant between them — and it is a property that would die silently if anything on that
path became `async`, since no test can schedule a broadcast into a window that does not exist.
`ws/replay.ts`'s docblock says so where somebody about to add an `await` would read it.

**`eventFanOut.test.ts` gained the assertion that replaces a claim LIVE-02 made in prose.** That
ticket's `rooms.ts` docblock said *nothing else may send a frame*, which was the reasoning stated too
widely and was never checked — and this ticket needed a second sender, since a reconnecting client is
replayed rows out of the log. What is actually load-bearing is narrower and now enforced: **an
`event` frame is composed in exactly one module**, `events/liveEventFrame.ts`, with the broadcast and
the replay both going through it. Verified by mutation in both directions (a second spelling in
`ws/replay.ts` fails it; so does moving the projection back into `recordEvent.ts`). The docblock was
rewritten to say what the test says.

**The replay is bounded in repetition as well as validated in value, which the review asked for and
D8 now states.** `afterSeq` was checked as a *number* — non-integer, negative and non-numeric refuse
the whole frame — and unbounded in *how often*: `rooms.join` is idempotent but a replay is not, so a
Member of one table could loop a 60-byte frame into an index read plus up to two hundred row reads,
parses and sends, synchronously, on the process serving every other table. `caughtUp` in
`ws/replay.ts` allows one catch-up per room per connection, which costs a legitimate client nothing
(a reconnect is a new connection) and is covered both ways: *should catch a connection up on one
room exactly once* and *should still catch that connection up on a different room*.

**Two structural claims that are not behavioural tests, recorded because they are the enforcement.**
`liveSocket.test.ts`'s *should be reachable from no store and no persistence service* walks
`stores/` and `services/` and asserts the set of modules naming the **connection** is exactly the
connection itself — v3 Req 44.9's structural half, since every action goes store → `characterSync` →
`api` and none of them may consult a socket before writing. Beside it, `characterStore.table.test.ts`
mocks `services/liveSocket` to **throw**, so a write path that ever reached for it would fail rather
than degrade.

**`yarn run check` is clean** (834 modules, 4298 dependencies cruised — 820/4237 at LIVE-02) and
`npx tsc --noEmit` is at its documented 2-error baseline.

**`fallow audit --base main` charged two complexity findings, both `introduced: true`, and both were
split rather than suppressed.** `decodeFrame` was measured mid-build at 11 cyclomatic / 12 cognitive
with a **37.1 CRAP** as soon as the subscribe grew a second field to read, and shed `decodeSubscribe`
— what a *subscribe* is made of being a different subject from what a *frame* is made of.
`SessionLobby` was measured at **16 cognitive**, one past the threshold, when every row gained a
presence badge, and shed `MemberList` — the branch that chooses between a list and a caption belongs
with the list, beside the `MemberRow` and `Departed` seams the file already had. Both readings above are **intermediate states that the shipped tree does
not reproduce** — measurements this build took on its own way through, recorded the way DM-03's entry
records its 18/19 first draft. After both splits the audit's complexity list holds **one** finding:
`useCharacterSheet`, which comes back `introduced: false` because the finding predates the diff — but
**this ticket did move it**, 16/19 → **17/21** (103 → 113 lines, 9 → 10 hooks) on the selector and
the one ternary that answers *which table, if any, is this sheet at*. Its hotspot row says so
plainly; the earlier claim that this ticket did not move it was wrong and is corrected there.

`fallow dead-code` reports nothing this ticket introduced — the two standing findings
(`RulesetHomeKind`, and the `fallow` dependency itself) are both inherited. One advisory styling line
names `CharacterSheet.tsx:200`'s `grid-cols-[minmax(0,1.7fr)_minmax(0,1fr)]` as *introduced drift*;
it is the sheet's pre-existing two-column rail, which moved eight lines down when the connection
notice went in above it, and no arbitrary value was written by this ticket.
...
| `src/server/repositories/eventRepository.test.ts` | 20.7 | TICKET-AUTH-03's run | 3 commits, 387 churn, 0.20 density | ▲ **Accelerating — TICKET-LIVE-03** (20.7 at AUTH-03, **9.8 now**, density 0.21 → 0.20). The score more than halved while the tag stayed, which is the honest shape of a *scale* change rather than a file change: the six-month window now holds far more churn elsewhere, and this file's own third commit is four `it`s in a new `describe`. **The visit is the smallest kind: a read function with no branches.** `latestEventSeq` asks the `(session_id, seq)` index for one row so that `replayTo` can decide whether a gap is worth replaying *before* fetching it — a client gone for an hour would otherwise have the server read two thousand rows in order to conclude it should not have. What would earn a real tag is a ticket that has to change how the **append** is tested, rather than adding a read beside it |
...
| `src/client/components/sessions/useSessionsManager.ts` | 12.5 | TICKET-CHAR-04's run | 4 commits, 178 churn, 0.09 density, 2 fan-in | ▲ **Accelerating — TICKET-CHAR-04** — re-measured at GAM-03's closeout and **falling**: 12.5 → 9.4, density 0.12 → 0.09. A fourth commit added 13 lines and the file got *easier*, because the invitations state landed as its own `waiting` list rather than as branches inside the games one. **Re-measured at DM-04: 9.4 → 6.6, still falling.** DM-04 took two hooks *out* of it — `useSessionMembers` and `useSessionCharacters` moved down into `useSessionRoster`, because a surface answering *who is here* and *what are their numbers* in one list needs them together and the manager was handing them to two components that each knew half. A composer that composes one thing instead of three is the cheapest kind of shrinking |
...
| `src/client/stores/characterStore.table.test.ts` | 10.1 | TICKET-DM-01's run | 12 commits, 698 churn, 0.12 density | ▼ Cooling — **cooled by TICKET-LIVE-03** (10.1 at DM-01, **24.5 now, velocity ▼**). The score is up and the velocity has turned, which is this table's benign combination: twelve commits of accumulated churn against a visit that added **one `describe` of two cases** and a module mock. The cases are v3 Req 44.9's behavioural half — a Player's action and a DM's both landing over HTTP with `services/liveSocket` stubbed to **throw** — and the mock is inert today, which is the claim rather than a gap: nothing under `stores/` imports the socket, asserted structurally in `services/liveSocket.test.ts`. The file still exists so `characterStore.test.ts` never has to change (the milestone's fifth Definition-of-Done rule), so growth here is the rule being honoured rather than a smell |
...
| `src/client/components/sessions/SessionsPanel.tsx` | 3.1 | TICKET-CHAR-04's run | 4 commits, 76 churn, 0.03 density | ▲ **Accelerating — TICKET-CHAR-04** — re-measured at TICKET-GAM-03's closeout: a fourth commit and **one line** of churn, score unmoved at 3.1. The panel composes rather than does, so each new surface costs it a `<Panel …/>` and nothing else — the number to watch is the day one of them arrives with a branch. **Re-measured at DM-04: 3.1 → 2.0.** That day still has not come; DM-04 replaced two surfaces with one and the panel's share of it was deleting a line |

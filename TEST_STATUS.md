...
_Last verified: 2026-08-29 (`npx vitest run`) at **TICKET-INV-05 — composed items: the record and
the engine**, the current count-setter at **3423**.
...
Split across **200 files**: `server` in node, everything else in happy-dom.
...
`dropItem.ts` ↔ `takeItem.ts`, is **inherited**: it is the route boilerplate every module in
